...
// Should work: x = Zero
printfn "%A" (solve [ Succ(Variable "x"), Succ(Zero) ])
// Should faild: S(Z) <> Z
// printfn "%A" (
// solve
...
printfn "%A" (solve [ Variable "x", Succ(Succ(Variable "z")); Succ(Variable "z"), Succ(Zero) ])

// Not done: Need occurs check
printfn "%A" (solve [ Variable "x", Succ(Variable "x") ])
